...
private:
  ...
  vector<SNode *> copyHeadVector ( const SNode & other );
};
...
private:
  ...
  bool containsHelper( SNode* prev, int val , SNode* curr, int lvs) const;

  //helper function add by Choi Ka Hin
  void addHelper(  SNode* pre, SNode* cur, int lvs, SNode* addMe);

  //helper function to find path
  void findPath( int val, SNode* prev, SNode* curr,unordered_map<int, SNode*> &path, int lvs);

  // helper function for remove function
  bool removeHelper(int val, int lvs, SNode* prev, SNode* curr);
